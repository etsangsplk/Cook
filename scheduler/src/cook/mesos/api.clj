...
(defn render-jobs-for-response
  [conn fid retrieve-url-path-fn ctx]
  (mapv (partial fetch-job-map (db conn) fid retrieve-url-path-fn) (::jobs ctx)))


;;; On GET; use repeated job argument
(defn read-jobs-handler
  [conn fid task-constraints gpu-enabled? is-authorized-fn retrieve-url-path-fn]
  (base-cook-handler
    {:allowed-methods [:get]
     ...
     :handle-ok (partial render-jobs-for-response conn fid retrieve-url-path-fn)}))
...
;;
(defn main-handler
  [conn fid mesos-pending-jobs-fn retrieve-url-path-fn
   {:keys [is-authorized-fn task-constraints] gpu-enabled? :mesos-gpu-enabled :as settings}]
  (->
   (routes
    (c-api/api
     ...
     (c-api/context
      ...
      (c-api/resource
       {:get {:summary "Returns info about a set of Jobs"
              ...
              :handler (read-jobs-handler conn fid task-constraints gpu-enabled? is-authorized-fn retrieve-url-path-fn)}
        ...
        :delete {:summary "Cancels jobs, halting execution when possible."
                 ...
                 :handler (destroy-jobs-handler conn fid task-constraints gpu-enabled? is-authorized-fn retrieve-url-path-fn)}}))
    ...
    (ANY "/list" []
         (list-resource (db conn) fid is-authorized-fn retrieve-url-path-fn)))
